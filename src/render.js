  ...
  o+='<div class="callout '+cls+'"><h3>'+head+' <span class="tag t-warn">'+L("참고 지표 · 근거 약함","indicator · weak evidence")+'</span></h3>'+
   ...
   mv.keys.map(k=>{const v=mv.c[k],st=v>=TH[k]?"🚫":v>=THW[k]?"⚠️":"✅";
     return "<tr><td>"+CH_NAME[k]+"</td><td class=\"ratio\">"+v.toFixed(0)+
       "</td><td>"+TH[k]+' <span class="cap">'+L("(가능 ","(feasible ")+FEAS[k][0]+"~"+FEAS[k][1]+L(" · 근거 "," · support ")+SUP[k]+L("건)",")")+"</span></td><td>"+st+"</td></tr>";}).join("")+
   "</tbody></table>"+
   ...
   '<div class="callout co-warn"><h3>'+L("⚠️ 이건 메커니즘 모델이 아닙니다","⚠️ This is not a mechanics model")+'</h3>'+
    "<p>"+L("세 지표는 밴드표 12점(카운터 7·밴드 5)을 <b>자유 파라미터 3개로 재현하도록 맞춘 곡선 적합</b>입니다. 게임 물리에서 유도한 값이 아닙니다.","The three channels are a <b>curve fit</b>: 12 labelled points (7 counters, 5 bans) reproduced with 3 free parameters. They are not derived from game physics.")+"</p>"+
    ...
    "<p>"+L("<b>표에 있는 상황이면 시트를 보세요.</b> 이 지표는 표에 없는 비율을 만났을 때의 참고용입니다. 가능 구간 <b>대보병 41~50 · 상성 31~40 · 후열 21~30</b>의 중앙값을 썼고, 후열은 밴드 4건이 지지(견고)·나머지 둘은 각 1건(취약)입니다.","<b>If the situation is in the table, read the sheet.</b> This indicator is for ratios the table does not cover. The values are midpoints of the feasible ranges <b>anti-infantry 41-50 · counter deficit 31-40 · back-line 21-30</b>; the back-line channel rests on 4 bans (robust), the other two on 1 each (fragile).")+"</p>"+
    "</div>"+
   "</div>";
  o+="</div>";}
