...
/* 청크 자르기 — <b>통행료 + 값 있는 마일스톤 하나</b>.
   값 0 짜리 마일스톤(아레나 전용 탐험, 이 병종이 안 쓰는 축)은 건너뛸 수 없으므로 그다음
   값 있는 마일스톤에 붙여 한 덩어리로 만든다. 그래서 "Lv.40 에서 멈춤"은 선택지가 아니다.
   모든 방향의 가중이 1이면 이 자르기는 예전의 "탐험+다음 원정" 과 정확히 같다.

   ⚠️ 값 있는 마일스톤끼리는 <b>합치지 않는다.</b> 예전에는 누적 (미스릴,값) 의 위쪽 오목
      껍질로 잘랐다 — 효율이 단조 감소해서 그리디가 깔끔했지만, 값이 낮은 마일스톤이 뒤의
      높은 것에 흡수되면서 <b>한 조각을 +20 에서 +100 까지 140 미스릴로 한 번에 올리는</b>
      덩어리가 생겼다. 순서표를 위에서부터 따라가는 사람에게는 그 자리에서 미스릴이 통째로
      묶인다 — 예산 300 에서 최적 270 대비 210 밖에 못 냈다(단일 청크는 270 로 최적).
      원본 우선순위표가 단계별(+20 전부 → +60 전부 → +100)로 도는 것과도 어긋났다.
      껍질을 버리고 값 있는 마일스톤마다 끊으면 순서표가 단계별로 돌고, 정차역이 살아나서
      gearBudgetPick 의 DP 도 <b>모든 마일스톤 정차역</b>을 후보로 갖게 된다.

   대가: 조각 안에서 효율이 단조가 아니다. 궁병 고글은 Lv.60(방어 0.75) 이 Lv.100(공격 1.0)
   보다 효율이 낮아 순서표에서 효율이 한 번 도로 올라간다 — 60 을 지나야 100 에 닿기 때문이라
   순서 자체는 옳다. 그 대신 순서표를 그대로 따라갔을 때의 손실이 최대 60%p 에서 25%p 로 줄었다.
...
function gearCut(atoms){
 const out=[]; let from=0,m=0,v=0;
 for(let i=0;i<atoms.length;i++){
  m+=atoms[i].mithril; v+=atoms[i].value;
  if(v>0){out.push({from,to:i,eff:v/m}); from=i+1; m=0; v=0;}
 }
 return out;
}
...
/* 예산 안에서 최선인 조합을 정확히 고른다.
   ⚠️ 효율 내림차순 그리디(④ 순서표)는 <b>청크 경계에서만</b> 최적이다. 임의 예산에서는 다음 청크가
...
const EXPEDITION_LEVELS=GEAR_MS.filter(function(m){return m.tier==="expedition";})
 .map(function(m){return m.level;});
const LEFTOVER_STAGE=GEAR_MS.length+1;
...
function gearPlan(input){
 ...
 GEAR_TROOP_ORDER.forEach((troop,ti)=>GEAR_SLOT_ORDER.forEach((slot,si)=>{
  ...
  const cut=gearCut(atoms);
  // 값이 하나도 없는 꼬리는 "완성용" 한 덩어리로 맨 뒤에 붙인다. 효율 0 이라 다른 모든
  // 청크 뒤로 밀리고, 화면에는 별도 구간으로 뜬다 — 미스릴만 쓰고 얻는 게 없는 자리다.
  const tail=cut.length?cut[cut.length-1].to+1:0;
  if(tail<atoms.length)cut.push({from:tail,to:atoms.length-1,eff:0,leftover:true});
  const chunks=cut.map(function(seg,seq){
   ...
   return {troop,slot,side,seq,ti,si,
    // 단계 = 이 청크가 여는 원정 마일스톤이 몇 번째인가(+20 → 0 · +60 → 1 · +100 → 2).
    // 아레나 정차역은 그 뒤, 완성용 꼬리는 맨 뒤.
    stage:seg.leftover?LEFTOVER_STAGE:(last.tier==="expedition"?EXPEDITION_LEVELS.indexOf(last.level):EXPEDITION_LEVELS.length),
    toLevel:last.level,
 ...
 // ── 정렬 기준 ──
 // 매번 조각들의 <b>맨 앞 청크</b>만 후보로 놓고 뽑는다. 앞을 내다보지 않는다 —
 // 뒤 청크의 효율이 더 높다고 앞 청크를 끌어올리면(옛 껍질이 그랬다) 한 조각에 미스릴이
 // 몰려 임의 예산에서 손해가 커진다. 실측으로 확인했다.
 //
 // 1순위가 <b>효율이 아니라 단계</b>다 — +20 을 전부 → +60 을 전부 → +100.
 // 효율만 보면 궁병 좌 계열의 +60(방어 0.75 · eff .45)이 같은 조각의 +100(공격 1.0 · eff .556)
 // 보다 낮아서, 그 조각만 +100 까지 올라가고 나머지가 +20 에 머무는 순서가 나온다.
 // 원본 우선순위표는 단계별로 돌고(9~16 = +20 · 17~24 = +60 · 25~32 = +100), 실측해도
 // 임의 예산 손실이 42.5 → 40%p 로 오히려 줄었다. 근거는 표와 실측이지 효율 정리가 아니다.
 // ⚠️ 아레나 정차역(탐험)은 이 단계 밖이다 — 단계에 끼우면 실전 기여 0 짜리 Lv.40 을
 //    다른 조각의 Lv.60 보다 먼저 사게 된다. 그래서 원정 단계 뒤로 따로 뺀다.
 ...
 const cmp=(a,b)=>(a.stage-b.stage)||(b.eff-a.eff)||(tw[b.troop]-tw[a.troop])||(isMain(b)-isMain(a))||
 ...
 for(;;){
  ...
  steps.push({troop:c.troop,slot:c.slot,side:c.side,seq:c.seq,stage:c.stage,
