...
function gearRender(plan,input){
 ...
 // 순서대로 자르면 다음 청크가 커서 안 들어갈 때 미스릴이 논다. 예산 안에서 최선인 조합을
 // 정확히 풀어서 고른다 — 그래서 여기 뜨는 목록은 위 순서의 앞부분이 아닐 수 있다.
 var pick=full?null:gearBudgetPick(plan.steps,budget);
 var shown=plan.steps.map(function(s,i){return {s:s,i:i};})
   .filter(function(x){return full||pick.set.has(x.i);});
 var shownM=shown.reduce(function(a,x){return a+x.s.mithril;},0);
 var trunc=0,cut=0;
 plan.steps.forEach(function(s){if(s.cumMithril<=budget){trunc+=s.useful;cut++;}});
 var bn=gEl("gBudgetN");
 if(bn)bn.innerHTML=full
  ? L("전체 "+plan.steps.length+"스텝 · 미스릴 "+T.mithril+" · 실제 쓸모 +"+Math.round(T.useful)+"%p",
      "All "+plan.steps.length+" steps · "+T.mithril+" mithril · usable +"+Math.round(T.useful)+"%p")
  : L("미스릴 <b>"+budget+"</b> 로 살 수 있는 <b>최선의 조합</b> — "+shown.length+"스텝 · 미스릴 "+shownM+
      " · 실제 쓸모 <b>+"+Math.round(pick.value)+"%p</b>"+
      (pick.value>trunc+1e-9?" <span class=\"tag t-ok\">순서대로 "+cut+"스텝만 자르는 것보다 +"+
        Math.round(pick.value-trunc)+"%p</span>":""),
      "<b>Best combination</b> for <b>"+budget+"</b> mithril — "+shown.length+" steps · "+shownM+
      " mithril · usable <b>+"+Math.round(pick.value)+"%p</b>"+
      (pick.value>trunc+1e-9?" <span class=\"tag t-ok\">+"+Math.round(pick.value-trunc)+
        "%p over taking the first "+cut+" in order</span>":""));
 ...
 if(!plan.steps.length){
  ...
  b+='<div class="xscroll"><table id="gSteps"><thead><tr><th>#</th><th></th><th>'+L("조각","Piece")+'</th><th>'+L("좌우","Side")+
   ...
   "</tbody></table></div>";
  if(!full)
   b+='<p class="note">'+L("<b>#</b> 는 전체 순서에서의 번호입니다. 예산 안에서 최선인 조합을 정확히 골랐기 때문에 <b>앞에서부터 잘라낸 목록이 아닙니다</b> — 번호가 건너뛸 수 있습니다. 슬라이더를 끝까지 올리면 전체 "+plan.steps.length+"스텝이 순서대로 보입니다.",
     "<b>#</b> is the position in the full order. This is the <b>optimal combination</b> for the budget, not the first N of the list, so numbers may skip. Raise the slider to see all "+plan.steps.length+" steps in order.")+"</p>";
 }
 b+="</div>";
